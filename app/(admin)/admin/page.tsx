  ...
  return <div className="opsDashboard">
    ...
    <section className="opsLayout">
      <article className="opsPanel opsPanelLarge"><div className="opsPanelHeader"><div><h2>Decision Queues</h2><p>Fast access to review surfaces that control platform quality and cash movement.</p></div></div><div className="opsQueueList">
        <QueueRow label="Mission Proof Review" detail="Approve or reject growth partner submissions" value={s.pendingCompletions.toLocaleString()} href="/admin/completions" tone="#F97316" />
        <QueueRow label="Business Campaign Review" detail="Check content, targeting, budget, and launch readiness" value={s.pendingCampaigns.toLocaleString()} href="/admin/campaigns" tone="#F5A623" />
        <QueueRow label="Withdrawal Operations" detail="Verify and process growth partner payout requests" value={s.withdrawalsPending.toLocaleString()} href="/admin/withdrawals" tone="#EF4444" />
      </div></article>
    ...
    <section className="opsLayout bottom">
      <article className="opsPanel opsPanelLarge"><div className="opsPanelHeader"><div><h2>Platform Flow</h2><p>How work is moving through the business-to-growth partner pipeline.</p></div></div><div className="opsFlow"><div><span>Business campaigns</span><strong>{totalCampaigns.toLocaleString()}</strong></div><div><span>Live missions</span><strong>{s.activeTasks.toLocaleString()}</strong></div><div><span>Growth Partner submissions</span><strong>{s.completionsTotal.toLocaleString()}</strong></div><div><span>Rewards released</span><strong>{fmt(s.qltAwarded)} QLT</strong></div></div></article>
      <article className="opsPanel"><div className="opsPanelHeader"><div><h2>New Growth Partners</h2><p>Latest user registrations.</p></div></div><div className="opsUserList">{s.recentUsers.length === 0 ? <p>No growth partners yet.</p> : s.recentUsers.map(user => <div key={`${user.email}-${String(user.created_at)}`}><span>{(user.full_name || user.email || "U").slice(0, 1).toUpperCase()}</span><div><strong>{user.full_name || "Unnamed growth partner"}</strong><small>{user.email}</small></div><time>{timeLabel(user.created_at)}</time></div>)}</div></article>
    </section>

    <section className="opsPanel"><div className="opsPanelHeader"><div><h2>Admin Shortcuts</h2><p>Common operational destinations.</p></div></div><div className="opsShortcutGrid"><Link href="/admin/users">Manage growth partners</Link><Link href="/admin/tasks">Mission inventory</Link><Link href="/admin/logs">Audit logs</Link><Link href="/admin/config">Economy settings</Link></div></section>
    {admin.role==="super_admin"&&<DataManagement />}
  </div>;
}
