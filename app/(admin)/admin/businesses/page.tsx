  ...
  return (
    <span
      style={{
        ...
        fontSize: 12,
        fontWeight: 700,
        background: active ? "#e8f5e9" : "#ffebeb",
        color: active ? "#2e7d32" : "#cc0000",
  ...
  async function updateBusinessStatus(business: Business) {
    ...
    try {
      const res = await fetch("/api/admin/businesses", {
        ...
        body: JSON.stringify({ id: business.id, action: shouldActivate ? "activate" : "suspend" }),
      });
          ...
          <tbody>
            {loading ? (
              ...
              businesses.map((business) => {
                ...
                return (
                  <tr key={business.id} style={{ background: active ? "transparent" : "#fff8f8" }}>
                    <td style={TD}>{business.id}</td>
                    <td style={{ ...TD, fontWeight: 600, color: "#222" }}>
                      <div style={{ display: "grid", gap: 5 }}>
                        <span>{business.name}</span>
                        <small style={{ color: "#b56d00", fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.4 }}>Business account</small>
                      </div>
                    </td>
                    ...
                    <td style={{ ...TD, minWidth: 210 }}>
                      <div style={{ display: "grid", gap: 4, fontSize: 12, lineHeight: 1.35 }}>
                        <span><strong style={{ color: "#555" }}>Industry:</strong> {business.industry || "Not set"}</span>
                        <span><strong style={{ color: "#555" }}>Website:</strong> {business.website || "Not set"}</span>
                        <span><strong style={{ color: "#555" }}>Reserved:</strong> {Number(business.reserved_budget).toLocaleString()} QLT</span>
                      </div>
                    </td>
                    <td style={TD}>
                      <div style={{ display: "grid", gap: 7 }}>
                        <StatusPill active={business.email_verified} label={business.email_verified ? "Email verified" : "Email pending"} />
                        <StatusPill active={business.onboarding_completed} label={business.onboarding_completed ? "Onboarded" : "Onboarding open"} />
                      </div>
                    </td>
                    <td style={{ ...TD, color: "#5f6876" }}>{new Date(business.created_at).toLocaleDateString()}</td>
                    <td style={TD}>
                      <StatusPill active={active} label={active ? "Active" : "Suspended"} />
                    </td>
                    <td style={TD}>
                      <button
                        onClick={() => updateBusinessStatus(business)}
                        ...
                        style={{
                          ...
                          fontSize: 12,
                          fontWeight: 700,
                          background: active ? "#cc0000" : "#1AEF22",
                          color: active ? "#fff" : "#000",
                          opacity: actionLoading === business.id ? 0.6 : 1,
                        }}
                      >
