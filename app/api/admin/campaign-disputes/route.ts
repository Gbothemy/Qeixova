...
export async function GET(req: NextRequest) {
  if (!await checkAdminAuth(req,"disputes.read")) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
...
export async function PATCH(req: NextRequest) {
  if (!await checkAdminAuth(req,"disputes.manage")) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
